...
# Audio files
original_path = './wav2vec/ori.wav'
initial_adv_path = './wav2vec/init_adv.wav'
    ...
    def bin_search(self, x, x_adv, loss):     # Binary search, roughly approaching the decision boundary.
        ...
        while (count_b < self.b):
            ...
            if (new_fitness < old_fitness):
                ...
                x_like = mid.copy()
            count_b += 1
        print('current time: ', self.t, '   bin search, get loss: ', old_fitness)
        ...
        for sub in range(self.l):
            ...
            if (current_fitness < current_best_fitness):   
                ...
                if (self.adv_count % 20 == 0):
                    filename = './wav2vec/tempadv/thisisatest/' + str(self.t) + '.wav'
        ...
        while (self.t < self.T):
            ...
            for group in groups:
                ...
                for sub in range(self.l):
                    ...
                    current_fitness = loss(x, solu)
                    if (current_fitness < current_best_fitness):
                        print('current time: ', self.t, '   sampled a better noise, loss: ', current_fitness)    
...
def main():
    ...
    final_adv = occam.attack( x, pad_x, wav2vec_loss )
    wavwrite('./wav2vec/final_adv_thisisatest.wav', final_adv)
